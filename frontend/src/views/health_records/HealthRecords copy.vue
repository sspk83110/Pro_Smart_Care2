        ...
        <v-card style="background-color: #ffffff; color: black">
          ...
          <v-card-text>
            <v-row>
              <v-col cols="12" sm="6">
                <v-text-field
                  v-model="currentDateTimeFormatted"
              ...
              <v-col cols="12" sm="8">
                <v-btn-toggle
                  ...
                  base-color="#e0e0e0"
                >
                  <v-btn value="present"> เข้าเรียน </v-btn>
                  <v-btn value="home"> กลับบ้าน </v-btn>
                  <v-btn value="absent"> ขาด </v-btn>
                  <v-btn value="leave"> ลา </v-btn>
                  <v-btn value="sick"> ป่วย </v-btn>
                </v-btn-toggle>
                <pre class="pt-2">{{ studentStatus }}</pre>
              </v-col>
              <v-col cols="12" sm="6">
                <v-select
...
const search = ref("");
const studentStatus = ref("present");
...
const formatThaiDate = (date) => {
  ...
  const year = date.getFullYear() + 543; // ปีพุทธศักราช (เพิ่ม 543)
...
const updateDateTime = () => {
  const now = new Date();
  currentDateTimeFormatted.value = formatThaiDateTime(now);
};
...
onMounted(() => {
  ...
  updateDateTime(); // เรียกครั้งแรกเพื่อให้แสดงทันที
  intervalId = setInterval(updateDateTime, 1000);
});

onUnmounted(() => {
  // ล้าง setInterval เมื่อคอมโพเนนต์ถูกถอดออกจาก DOM เพื่อป้องกัน Memory Leak
  if (intervalId) {
